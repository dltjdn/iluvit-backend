...
public interface BoardBookmarkRepository extends JpaRepository<Bookmark, Long> {
    ...
    */
    @Query("select p from Post p join fetch p.board b where p.id in " +
            ...
            "group by p.board.id) and p.board.id = b.id and p.user not in :blockedUsers ")
    List<Post> findPostByBoard(@Param("userId") Long userId, @Param("blockedUsers") List<User> blockedUsers);
