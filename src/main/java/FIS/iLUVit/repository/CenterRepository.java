...
public interface CenterRepository extends JpaRepository<Center, Long>, CenterRepositoryCustom {
    ...
    @Query("select new FIS.iLUVit.repository.dto.CenterBannerDto(center.id, center.name, center.signed, center.recruit, avg(review.score), prefer.center.id, center.profileImagePath) " +
            "from Center center " +
    ...
    @Query("select new FIS.iLUVit.repository.dto.CenterBannerDto(center.id, center.name, center.signed, center.recruit, avg(review.score), center.profileImagePath) " +
            "from Center center " +
            "left join center.reviews as review " +
            "where center.id=:id " +
            "group by center")
