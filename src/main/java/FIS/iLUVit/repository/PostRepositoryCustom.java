...
public interface PostRepositoryCustom {
     ...
     */
    Slice<PostResponse> findInCenterByKeyword(Collection<Long> centerIds, String keyword, Pageable pageable);

    /*
        시설 id가 주어진 시설 id와 동일한지 확인하고 keywordContains 메서드를 호출하여 키워드 검색을 적용하여 게시글 미리보기 DTO 객체를 불러옵니다.
     */
    Slice<PostResponse> findByCenterAndKeyword(Long centerId, String keyword, Pageable pageable);

    /*
        시설 id가 주어진 시설 id 값과 같은지 확인하고 keywordContains 메서드를 호출하여 키워드 검색을 적용하여 게시글 미리보기 DTO 객체를 불러옵니다.
     */
    Slice<PostResponse> findByBoardAndKeyword(Long boardId, String keyword, Pageable pageable);

    /*
        시설 id가 주어진 시설 id와 같고 게시글 엔티티의 하트 개수가 주어진 하트 개수보다 크거나 같은지 확인하여 게시글 미리보기 DTO를 불러옵니다.
     */
    Slice<PostResponse> findHotPosts(Long centerId, Integer heartCnt, Pageable pageable);
}
